...
Then open `http://localhost:4173`. This panel manages the dedicated Feed News records created by the automation through `/api/feed-news`; it does not use the separate Brokket `/api/more-pages/news` section. The panel token is stored only in the browser and is never added to this repository.
